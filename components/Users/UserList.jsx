          ...
          <div className="flex items-center gap-2 flex-wrap">
            {hasNewUsers && (
              ...
              >
                ...
                <span className="hidden sm:inline">New users available</span>
      ...
      <div className="flex flex-col gap-3 sm:gap-4 mt-4 sm:mt-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-border/50 bg-surface-secondary text-primary placeholder:text-muted focus:outline-none focus:bg-surface transition-colors text-sm sm:text-base"
          />
        </div>
        {/* Status Filter Tabs - Scrollable on mobile */}
        <div className="flex items-center bg-surface-secondary rounded-lg p-1 overflow-x-auto scrollbar-hide">
          {[
            { value: 'all', label: 'All' },
          ...
          <table className="w-full min-w-[700px]">
            <thead>
              <tr className="border-b border-border bg-surface-secondary">
                <th className="p-2 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-muted uppercase cursor-pointer whitespace-nowrap" onClick={() => handleSort('name')}>
                  User {sortConfig.sortBy === 'name' && (sortConfig.sortOrder === 'asc' ? '↑' : '↓')}
                </th>
                <th className="p-2 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-muted uppercase whitespace-nowrap">Email</th>
                <th className="p-2 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-muted uppercase whitespace-nowrap">Credits</th>
                <th className="p-2 sm:p-3 text-center text-[10px] sm:text-xs font-medium text-muted uppercase whitespace-nowrap hidden md:table-cell">Profiles</th>
                <th className="p-2 sm:p-3 text-center text-[10px] sm:text-xs font-medium text-muted uppercase whitespace-nowrap hidden md:table-cell">Analyses</th>
                <th className="p-2 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-muted uppercase cursor-pointer whitespace-nowrap hidden sm:table-cell" onClick={() => handleSort('createdAt')}>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-6 sm:p-8 text-center text-muted text-sm">
                  ...
                  >
                    <td className="p-2 sm:p-3">
                      <div className="flex items-center gap-2 sm:gap-3">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs sm:text-sm font-semibold flex-shrink-0">
                          {user.name?.charAt(0).toUpperCase() || '?'}
                        </div>
                        <div className="min-w-0">
                          <div className="font-medium text-primary text-sm truncate max-w-[120px] sm:max-w-none">{user.name || 'Unnamed'}</div>
                          <div className="text-[10px] sm:text-xs text-muted">{user.id.substring(0, 8)}...</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-2 sm:p-3 text-xs sm:text-sm text-primary truncate max-w-[150px]">{user.email || 'N/A'}</td>
                    <td className="p-2 sm:p-3">
                    ...
                    <td className="p-2 sm:p-3 text-center text-xs sm:text-sm text-muted hidden md:table-cell">
                    ...
                    <td className="p-2 sm:p-3 text-xs sm:text-sm text-muted hidden sm:table-cell whitespace-nowrap">
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-center gap-2 mt-4">
          <div className="flex items-center gap-1 sm:gap-2">
            <Button variant="secondary" size="sm" disabled={currentPage === 1} onClick={() => handlePageChange(0)} className="px-2 sm:px-3">
              <span className="hidden sm:inline">First</span>
              <span className="sm:hidden">«</span>
            </Button>
            <Button variant="secondary" size="sm" disabled={currentPage === 1} onClick={() => handlePageChange(pagination.offset - pagination.limit)} className="px-2 sm:px-3">
              <span className="hidden sm:inline">Previous</span>
              <span className="sm:hidden">‹</span>
            </Button>
          </div>
          <span className="px-3 sm:px-4 text-xs sm:text-sm text-muted whitespace-nowrap">
            {currentPage} / {totalPages}
          </span>
          <div className="flex items-center gap-1 sm:gap-2">
            <Button variant="secondary" size="sm" disabled={currentPage === totalPages} onClick={() => handlePageChange(pagination.offset + pagination.limit)} className="px-2 sm:px-3">
              <span className="hidden sm:inline">Next</span>
              <span className="sm:hidden">›</span>
            </Button>
            <Button variant="secondary" size="sm" disabled={currentPage === totalPages} onClick={() => handlePageChange((totalPages - 1) * pagination.limit)} className="px-2 sm:px-3">
